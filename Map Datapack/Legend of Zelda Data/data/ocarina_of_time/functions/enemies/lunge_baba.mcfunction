...
execute unless entity @s[tag=Stun] if entity @s[scores={timer=1}] run summon minecraft:marker ~ ~ ~ {Tags:["LungeBabaReset"],Age:-2147483648,Duration:-1,WaitTime:-2147483648}
...
execute unless entity @s[tag=Stun] if entity @s[tag=Attack] unless entity @a[distance=..6] run tp @s @e[type=marker,tag=LungeBabaReset,limit=1,sort=nearest]
execute unless entity @s[tag=Stun] if entity @s[tag=Attack] unless entity @a[distance=..6] run kill @e[type=marker,tag=LungeBabaReset,limit=1,sort=nearest]
