...
execute unless entity @s[tag=Stun] if entity @s[scores={timer=1}] run summon minecraft:marker ~ ~ ~ {Tags:["ArmosReset"],Age:-2147483648,Duration:-1,WaitTime:-2147483648}
...
execute unless entity @s[tag=Stun] if entity @s[scores={lifetime=6}] unless block ~ ~-.5 ~ #ocarina_of_time:armos facing entity @e[type=marker,tag=ArmosReset,sort=nearest,limit=1] feet rotated ~ 0 run tp @s ^ ^.6 ^.6 ~ 0
execute unless entity @s[tag=Stun] if entity @s[scores={lifetime=11..}] run scoreboard players set @s lifetime 0

#Reset
execute unless entity @s[tag=Stun] if entity @s[scores={lifetime=1..}] if entity @e[type=marker,tag=ArmosReset,distance=..0.5,sort=nearest,limit=1] run tag @s add Reset
execute if entity @s[tag=Reset] at @e[type=marker,tag=ArmosReset,sort=nearest,limit=1] run tp @s ~ ~ ~
execute if entity @s[tag=Reset] run kill @e[type=marker,tag=ArmosReset,limit=1,sort=nearest]
