...
execute unless entity @e[type=armor_stand,tag=OwlStand10] run summon minecraft:armor_stand 730.3 73 2761 {Tags:["OwlStand10","Npc"],Rotation:[0f],ArmorItems:[{},{},{},{id:infested_stone,Count:1b,tag:{CustomModelData:1}}],NoGravity:1b,Invisible:1b}
execute unless block 697 67 2764 minecraft:gold_block run scoreboard players set @e[type=armor_stand,tag=OwlStand10] text 1
execute if block 697 67 2764 minecraft:gold_block run scoreboard players set @e[type=armor_stand,tag=OwlStand10] text 2
...
execute if entity @a[tag=Adult] if entity @e[type=marker,tag=BeanSpot,tag=PlantBean9] run clone 734 67 2691 736 67 2693 738 71 2691
execute unless entity @a[tag=Adult] if entity @e[type=marker,tag=BeanSpot,tag=PlantBean9] run fill 739 71 2692 739 71 2692 minecraft:pumpkin_stem[age=6]
