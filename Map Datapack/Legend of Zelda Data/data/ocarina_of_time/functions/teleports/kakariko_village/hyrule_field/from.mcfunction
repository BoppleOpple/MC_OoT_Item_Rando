...
execute if entity @s[scores={LoadZone=1}] unless entity @s[scores={time=13001..23000}] if entity @a[tag=Adult] unless entity @e[type=marker,tag=Progress,tag=GotForestMedallion,tag=GotFireMedallion,tag=GotWaterMedallion,tag=!FinishKakariko] run function ocarina_of_time:music/kakariko_village
...
execute if entity @s[scores={LoadZone=15..}] if entity @a[tag=Adult] as @e[type=marker,tag=Progress,tag=GotForestMedallion,tag=GotFireMedallion,tag=GotWaterMedallion] unless entity @s[tag=FinishKakariko] run tag @a add KakarikoSheik
