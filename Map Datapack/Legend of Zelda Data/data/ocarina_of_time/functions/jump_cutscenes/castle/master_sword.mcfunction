...
execute unless entity @a[scores={click=0..}] run tag @e[type=marker,tag=Progress] add KokiriTunic
...
execute unless entity @a[scores={click=0..}] run item replace entity @a armor.chest with minecraft:leather_chestplate{display:{Name:"{\"text\":\"Kokiri Tunic\"}",color:40203},Unbreakable:1b,HideFlags:46,KokiriTunic:1b}
...
execute if entity @a[scores={click=78}] as @e[type=marker,tag=FakeSaria] unless entity @s[scores={text=6..}] run scoreboard players set @s text 6
