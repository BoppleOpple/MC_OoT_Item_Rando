...
execute if entity @a[scores={click=1}] run tag @e[type=marker,tag=Songs] add Scarecrow
...
execute if entity @s[tag=WrongSong] run tag @e[type=marker,tag=Songs] remove Scarecrow
...
execute if entity @s[tag=CorrectSong] run tag @e[type=marker,tag=Songs] remove Scarecrow
...
execute if entity @s[scores={GameTimer=200}] run tellraw @a {"text":"Look down when you play the ocarina...","color":"red"}
