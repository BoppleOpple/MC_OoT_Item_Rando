...
execute unless entity @s[tag=Revisit] if entity @a[scores={click=27}] run tag @e[type=marker,tag=Progress] add HalfMagic
...
execute if entity @s[scores={timer=41..}] as @e[type=marker,tag=FakeSaria] unless entity @s[scores={lifetime=10..}] run scoreboard players set @s lifetime 10
