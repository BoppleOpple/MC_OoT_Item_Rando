...
execute if entity @a[scores={click=3}] run tag @e[type=marker,tag=Progress] add BiggoronSword
execute if entity @a[scores={click=3}] run give @a minecraft:golden_sword{display:{Name:"\"Biggoron's Sword\""},HideFlags:63,Unbreakable:1,CanDestroy:["minecraft:grass","minecraft:oak_sign","minecraft:fern","minecraft:flower_pot","minecraft:azalea_leaves"],Enchantments:[{id:sharpness,lvl:19}],BiggoronSword:1b,MainHandItem:1b}
