...
execute if entity @a[scores={click=31..}] run tag @e[type=marker,tag=Progress] add GotSpiritMedallion
execute if entity @a[scores={click=31..}] unless entity @e[type=marker,tag=Progress,tag=GotShadowMedallion,tag=GotSpiritMedallion] as @e[type=marker,tag=FakeSaria] unless entity @s[scores={lifetime=23..}] run scoreboard players set @s lifetime 23
