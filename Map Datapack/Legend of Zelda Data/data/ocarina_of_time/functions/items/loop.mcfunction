...
#Hookshot
execute as @a[scores={UseItem=1..}] at @s unless entity @a[tag=PlayingSong] if entity @s[nbt={FallDistance:0.0f,SelectedItem:{tag:{Hookshot:1b}}}] unless entity @s[tag=NoHookshot] unless entity @e[type=marker,tag=Hookshot] run summon minecraft:marker ~ ~ ~ {Tags:["Hookshot"],Age:-2147483648,Duration:-1,WaitTime:-2147483648}
execute as @a[scores={UseItem=1..}] at @s unless entity @a[tag=PlayingSong] if entity @s[nbt={FallDistance:0.0f,Inventory:[{Slot:-106b,tag:{Hookshot:1b}}]}] unless entity @s[tag=NoHookshot] unless entity @e[type=marker,tag=Hookshot] run summon minecraft:marker ~ ~ ~ {Tags:["Hookshot"],Age:-2147483648,Duration:-1,WaitTime:-2147483648}
...
#Iron Boots
execute as @a at @s if entity @s[nbt={Inventory:[{Slot:100b,tag:{IronBoots:1b}}]}] unless entity @e[type=marker,tag=Hookshot] run function ocarina_of_time:items/boots/iron_boots
...
#Pants
item replace entity @a armor.legs with minecraft:golden_leggings{Unbreakable:1b,display:{Name:"{\"text\":\"\"}"},Enchantments:[{id:depth_strider,lvl:2},{id:respiration,lvl:1},{id:binding_curse,lvl:1}],HideFlags:63}
