...
execute as @e[type=marker,tag=CompassChest] at @s run function ocarina_of_time:items/compass/count

execute as @e[type=marker,tag=CompassChest1] at @s run tellraw @a ["",{"text":"There are "},{"score":{"name":"$empty_chests","objective":"Compass"},"color":"red"},{"text":" Chests remaining."}]

scoreboard players set $empty_chests Compass 0
kill @e[type=marker,tag=CompassChest]
