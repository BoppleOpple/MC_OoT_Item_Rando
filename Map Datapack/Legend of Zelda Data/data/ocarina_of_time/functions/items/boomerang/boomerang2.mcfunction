...
execute at @s positioned ~ ~ ~ as @e[type=marker,tag=RedJelly,distance=..3] run tag @s add Stun
...
execute at @s positioned ~ ~ ~ if entity @e[type=!player,type=!marker,type=!item,tag=!Boomerang,tag=!BoomerangStart,tag=!DekuNut,tag=!HintNavi,distance=..1.3] run scoreboard players set @s lifetime 51
