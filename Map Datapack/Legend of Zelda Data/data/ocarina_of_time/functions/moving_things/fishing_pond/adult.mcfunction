...
#Sinking Lure
execute unless entity @a[tag=SinkLure] if block 411 57 2718 minecraft:gold_block at @e[type=marker,tag=SinkSpot] if entity @a[distance=..1] run tellraw @a ["",{"text":"Wow! Is this what I think it is? Astonishing! You snagged a "},{"text":"sinking lure","color":"red"},{"text":"!"}]
execute unless entity @a[tag=SinkLure] if block 411 57 2718 minecraft:gold_block at @e[type=marker,tag=SinkSpot] if entity @a[distance=..1] run tag @a add SinkLure
