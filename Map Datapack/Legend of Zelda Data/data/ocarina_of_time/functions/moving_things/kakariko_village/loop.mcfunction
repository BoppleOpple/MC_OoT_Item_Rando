...
execute as @a positioned 1644 76 1949 unless entity @s[distance=..13] unless entity @s[tag=KakarikoSheik2] unless entity @e[type=marker,tag=Songs,tag=Rain] run weather clear
...
execute as @e[type=armor_stand,tag=Dampe2,scores={GameTimer=30},tag=Start] at @s run summon minecraft:silverfish ~ ~1 ~ {Tags:["Invisible","Fire"],NoAI:1b,Silent:1,Invulnerable:1,HasVisualFire:1b}
...
#Talon Returns
execute if entity @e[type=marker,tag=Progress,tag=GotEpona] run scoreboard players set @e[type=armor_stand,tag=VillageTalon] text 2
...
execute if entity @a[x=1624,y=42,z=2010,dx=8,dz=6] if entity @a[nbt={Inventory:[{id:"minecraft:music_disc_wait",Count:1b}]}] unless entity @a[x=1626,y=41,z=2012,dx=2,dz=2] run scoreboard players add @e[type=marker,tag=TimeBlock] timer 1

#Big Fairies
execute positioned 1617 51 1956 if entity @a[distance=..5] unless entity @e[type=marker,tag=BigFairy,distance=..1] unless entity @a[tag=GreenNaviSpot] run summon minecraft:armor_stand 1617 51 1956 {Tags:["GreenNavi"],Small:1b,Marker:1b,NoGravity:1b,ArmorItems:[{},{},{},{id:"minecraft:player_head",Count:1b,tag:{SkullOwner:{Id:[I;-102214788,162220947,-1666754575,1836136248],Properties:{textures:[{Value:"eyJ0ZXh0dXJlcyI6eyJTS0lOIjp7InVybCI6Imh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvNTljNTE1NDVkNTQ5NDczZWNjYWE2NzdiZTc3ZmM4OTNjZmEwNWVjMWYwM2I4NWMyZWJjMjM0MmM0YTlmOTdjMyJ9fX0="}]}}}}]}
execute positioned 1617 51 1956 if entity @a[distance=..5] unless entity @e[type=marker,tag=BigFairy,distance=..1] unless entity @a[tag=GreenNaviSpot] run tag @a add GreenNaviSpot
execute positioned 1617 51 1956 if entity @a[distance=..5] unless entity @e[type=marker,tag=BigFairy,distance=..1] if entity @a[nbt={Inventory:[{id:"minecraft:music_disc_stal",Count:1b}]}] run function ocarina_of_time:items/big_fairy/summon
...
#Hookshot
execute as @e[type=marker,tag=Hookshot] at @s unless entity @s[scores={Hookshot=200..}] run function ocarina_of_time:moving_things/kakariko_village/hookshot
execute as @e[type=marker,tag=Hookshot,x=1557.8,y=81.5,z=1932.5,dx=1,dy=1.5,dz=1] run tp @s 1558 82.3 1932.2
...
#Pots
execute as @e[type=marker,tag=Pots] run function ocarina_of_time:moving_things/kakariko_village/pots
