...
#Shoot
execute if entity @e[type=marker,tag=TwinrovaSpot,tag=!Cooldown,distance=..1] unless entity @e[type=zombie,tag=Koume,tag=Shoot] run tag @s add Shoot
...
execute if entity @s[tag=Shoot,scores={timer=50}] positioned ^-1.5 ^1.5 ^.5 run summon minecraft:marker ~ ~ ~ {Tags:["Twinrova","IceBeam"],Age:-2147483648,Duration:-1,WaitTime:-2147483648}
execute if entity @s[tag=Shoot,scores={timer=51..300}] unless entity @e[type=marker,tag=IceBeam,tag=Boom] positioned ^-1.5 ^1.5 ^.5 facing entity @e[type=marker,tag=IceBeam] feet run function ocarina_of_time:moving_things/spirit_temple/boss/ice
...
execute if entity @s[tag=Shoot,scores={timer=51..300}] as @a[tag=ReflectIce] at @s anchored eyes facing entity @e[type=zombie,tag=Kotake] eyes anchored feet positioned ^ ^ ^1 rotated as @s positioned ^ ^ ^-1 unless entity @s[distance=..1] run tag @e[type=marker,tag=IceBeam] add Boom
...
execute unless entity @s[tag=Finish] if entity @s[scores={lifetime=1}] facing entity @e[type=marker,tag=TwinrovaSpot,tag=!Cooldown,sort=random] feet rotated ~ 0 run tp @s ~ ~ ~ ~ 0
...
execute if entity @s[tag=Finish,scores={lifetime=341}] as @e[type=marker,tag=TwinrovaSpot] at @s run tp @s ~ ~1 ~
