...
execute unless entity @e[type=marker,tag=IceBeam,distance=..1] unless entity @a[distance=..1.5,nbt={SelectedItem:{id:"minecraft:shield",Count:1b}}] unless entity @a[distance=..1.5,nbt={Inventory:[{Slot:-106b,id:"minecraft:shield",Count:1b}]}] if block ~ ~ ~ minecraft:air positioned ^ ^ ^1 run function ocarina_of_time:moving_things/spirit_temple/boss/ice
