...
execute if score @s BlueRupee <= const BlueRupee at @a store result entity @e[type=item,nbt={Item:{Count:1b,id:"minecraft:lapis_lazuli",tag:{display:{Name:"{\"text\":\"Blue Rupee\"}"},Item:1b}}},distance=..2,limit=1] Item.Count byte 1 run scoreboard players get @e[type=marker,tag=Progress,limit=1] BlueRupee
