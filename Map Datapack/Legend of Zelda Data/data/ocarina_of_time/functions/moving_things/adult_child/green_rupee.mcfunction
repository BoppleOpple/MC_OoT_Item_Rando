...
execute if score @s GreenRupee <= const GreenRupee at @a store result entity @e[type=item,nbt={Item:{Count:1b,id:"minecraft:lime_dye",tag:{display:{Name:"{\"text\":\"Green Rupee\"}"},Item:1b}}},distance=..2,limit=1] Item.Count byte 1 run scoreboard players get @e[type=marker,tag=Progress,limit=1] GreenRupee
