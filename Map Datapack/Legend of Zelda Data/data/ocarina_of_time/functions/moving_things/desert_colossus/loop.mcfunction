...
execute if entity @a[tag=LeeverAttack] as @e[type=marker,tag=LeeverSpawn] at @s run function ocarina_of_time:moving_things/desert_colossus/leever_spawn
...
#Adult Bean Spot
execute if entity @a[tag=Adult] as @e[type=marker,tag=BeanSpot] at @a run function ocarina_of_time:moving_things/desert_colossus/bean_spot
...
execute unless entity @a[tag=Adult] if entity @a[x=399,y=55,z=392,dx=0,dz=0] unless entity @e[type=marker,tag=BeanSpot,tag=PlantBean3] if entity @a[nbt={SelectedItem:{tag:{MagicBean:1b}}}] positioned 399 55 392 run function ocarina_of_time:items/magic_bean
execute unless entity @a[tag=Adult] if block 399 55 392 minecraft:pumpkin_stem[age=6] as @e[type=marker,tag=BeanSpot] unless entity @s[tag=PlantBean3] run tag @s add PlantBean3
